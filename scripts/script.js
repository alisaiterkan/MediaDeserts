...
function filterHTML(filterID, viewsOptions, statesOptions, yearsOptions, publicationsOptions) {
	return '<div class="filter-row cf" id="filter-'+filterID+'"><div class="views-selector cf"><select name="views-'
		+filterID+'" id="views-'+filterID+'">'+viewsOptions+'</select></div><div class="states-selector cf"><select name="states-'+filterID
		+'" id="states-'+filterID+'" multiple="" style="display: none;">'+statesOptions+
		'</select></div><div class="year-selector cf"><select name="years-'+filterID+'" id="years-'+filterID
		+'" multiple>'+yearsOptions+'</select></div><div class="publication-selector cf"><select name="publications-'+
		filterID+'" id="publications-'+filterID+'" multiple>"'+publicationsOptions+'"</select></div></div>';
    }
...
$(document).ready(
	...
	function() {
		...
		}//end successful location load
		else {
			console.log("Location Finding Failed")
		}//end unsuccessful location load
		...
		});//endpageloadingmoduleuse
	}//end document.ready's inner function
);//end document.ready

//When a polygon is moused over, highlight the edge
function mouseoverPolygon(area){
	if(currentpolygon !== area) {
		area.setOptions({
			strokeOpacity: 0.5,
			fillOpacity: .90,
			strokeWeight: 1
...
function clickPolygon(area,  zipReportsHTML, zipDemographicsHTML){
	if(currentpolygon !== null) {
		...
		$('#infotable').html("Click on a colored area to see demographics information");
	}
	currentpolygon = area;
...
function initialize() {
	...
	//Open the loading module while changing
	$("#states-selector").change( function() {
			...
			$.ajax({
				...
				geocoder.geocode( {'address' : chosenState}, function(results, status) {
					...
					}//end geocode function parameter
				);//end geocoding

				for(var key in zippolygon){
					zippolygon[key].setMap(null);
				}//end map-setting for-loop
	...
	function ajaxCall(data) {
		...
		$(data).children('response').children('area').each(function() {
			...
			$(thisRow).children('reports').children('report').each(function() {
				...
				reportHTML = reportHTML + "<p class='open-link'><span class='open-link-newspaper-name'>" + reportData['name'] 
							+ "</span> <span class='open-link-report-date'>(" + reportData['reportPeriod'] 
							+ ")</span><span class='open-link-status'>+</span></p>";
				...
				reportHTML = reportHTML + "<tr><td>" + reportData['combinedaverage'] + "</td><td>" + reportData['mondaycirculation'] 
							+ "</td><td>" + reportData['tuesdaycirculation'] + "</td><td>" + reportData['wednesdaycirculation'] + "</td><td>" 
							+ reportData['thursdaycirculation'] + "</td><td>" + reportData['fridaycirculation'] + "</td><td>" 
							+ reportData['saturdaycirculation'] + "</td><td>" + reportData['sundaycirculation'] + "</td></tr></table>";
			...
			infoHTML = infoHTML + "<th><select name='demographics-1' id='demographics-1'>"
						+"<option value='population' selected>Population</option><option value='households'>"
						+"households</option><option value='medianAge'>medianAge</option><option value='pctBachelors'>"
						+"pctBachelors</option><option value='pctUnemployed'>pctUnemployed</option><option value='income'>"
						+"income</option><option value='householdSize'>householdSize</option><option value='pctHousingOwned'>"
						+"pctHousingOwned</option><option value='pctHousingRented'>pctHousingRented</option>"
						+"<option value='pctHousingVacant'>pctHousingVacant</option><option value='medianHomeValue'>"
						+"medianHomeValue</option><option value='pctProjectedGrowth'>pctProjectedGrowth</option>"
						+"<option value='pctHouseholdGrowth'>pctHouseholdGrowth</option><option value='pctIncomeGrowth'>"
						+"pctIncomeGrowth</option></select></th>";

			infoHTML = infoHTML + "<th><select name='demographics-2' id='demographics-2'><option value='population'>"
						+"Population</option><option value='households' selected>households</option><option value='medianAge'>"
						+"medianAge</option><option value='pctBachelors'>pctBachelors</option><option value='pctUnemployed'>"
						+"pctUnemployed</option><option value='income'>income</option><option value='householdSize'>householdSize"
						+"</option><option value='pctHousingOwned'>pctHousingOwned</option><option value='pctHousingRented'>"
						+"pctHousingRented</option><option value='pctHousingVacant'>pctHousingVacant</option>"
						+"<option value='medianHomeValue'>medianHomeValue</option><option value='pctProjectedGrowth'>"
						+"pctProjectedGrowth</option><option value='pctHouseholdGrowth'>pctHouseholdGrowth</option>"
						+"<option value='pctIncomeGrowth'>pctIncomeGrowth</option></select></th>";

			infoHTML = infoHTML + "<th><select name='demographics-3' id='demographics-3'><option value='population'>"
						+"Population</option><option value='households'>households</option><option value='medianAge' selected>"
						+"medianAge</option><option value='pctBachelors'>pctBachelors</option><option value='pctUnemployed'>"
						+"pctUnemployed</option><option value='income'>income</option><option value='householdSize'>householdSize"
						+"</option><option value='pctHousingOwned'>pctHousingOwned</option><option value='pctHousingRented'>"
						+"pctHousingRented</option><option value='pctHousingVacant'>pctHousingVacant</option>"
			...
			infoHTML = infoHTML + "<th><select name='demographics-4' id='demographics-4'><option value='population'>"
						+"Population</option><option value='households'>households</option><option value='medianAge'>"
						+"medianAge</option><option value='pctBachelors' selected>pctBachelors</option><option value='pctUnemployed'>"
						+"pctUnemployed</option><option value='income'>income</option><option value='householdSize'>householdSize"
						+"</option><option value='pctHousingOwned'>pctHousingOwned</option><option value='pctHousingRented'>"
						+"pctHousingRented</option><option value='pctHousingVacant'>pctHousingVacant</option>"
						+"<option value='medianHomeValue'>medianHomeValue</option><option value='pctProjectedGrowth'>"
			...
			infoHTML = infoHTML + "<th><select name='demographics-5' id='demographics-5'><option value='population'>"
						+"Population</option><option value='households'>households</option><option value='medianAge'>"
						+"medianAge</option><option value='pctBachelors'>pctBachelors</option><option value='pctUnemployed' selected>"
						+"pctUnemployed</option><option value='income'>income</option><option value='householdSize'>householdSize</option>"
						+"<option value='pctHousingOwned'>pctHousingOwned</option><option value='pctHousingRented'>pctHousingRented</option>"
						+"<option value='pctHousingVacant'>pctHousingVacant</option><option value='medianHomeValue'>medianHomeValue</option>"
						+"<option value='pctProjectedGrowth'>pctProjectedGrowth</option><option value='pctHouseholdGrowth'>pctHouseholdGrowth"
...
*/
	...
	// var center defines the central point of map on load, right now it's focused on NC coordinates
